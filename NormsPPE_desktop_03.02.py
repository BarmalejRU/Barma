...
def create_employee_window():
    ...
    def custom_focus_next_entry(event):
        current_widget = event.widget

        # Специальная обработка для поля "Структурное подразделение"
        if current_widget == entries["department"]:
            entries["hire_date"].focus_set()
            return "break"

        # Стандартное поведение для остальных полей
        next_widget = current_widget.tk_focusNext()

        # Пропускаем чекбоксы в цепочке перехода
    ...
    entries["patronymic"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["tab_number"].bind("<Return>", custom_focus_next_entry)

    # Поле "Структурное подразделение" с чекбоксом
    department_frame = Frame(left_frame)
    department_frame.pack(fill='x')
    Label(department_frame, text="Структурное подразделение:").pack(anchor='w')
    entries["department"] = Entry(department_frame, width=30)
    entries["department"].pack(side='left', fill='x', expand=True)
    entries["department"].bind("<Return>", custom_focus_next_entry)

    entries["department_check"] = Checkbutton(department_frame)
    entries["department_check"].var = BooleanVar()
    entries["department_check"]["variable"] = entries["department_check"].var
    entries["department_check"].pack(side='right', padx=5)
    entries["department_check"].tk_focusNext = lambda: entries["hire_date"]
    ...
    entries["hire_date"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["change_date"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["clothes_size"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["shoes_size"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["helmet_size"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["sizod_size"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["responsible_person"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["responsible_check"].tk_focusNext = lambda: entries["card_number"] if entries.get("card_number") else \
    entries["surname"]
    ...
    entries["card_number"].bind("<Return>", custom_focus_next_entry)
    ...
    entries["card_check"].tk_focusNext = lambda: entries["surname"]
    ...
    Button(button_frame, text="Закрыть", command=on_employee_window_close).pack(side='left', padx=5)

    # Устанавливаем фокус на первое поле
    entries["surname"].focus_set()
...
def validate_and_save():
    data = {
        ...
        "department_check": entries["department_check"].var.get()
    }
    errors = []
    ...
    missing_fields = [field for field, value in data.items() if
                        field not in ["Фамилия", "Имя", "Табельный номер", "Дата поступления", "Размер СИЗОД",
                                    ...
                                    "card_check", "department_check"] and not value]
    ...
    for key, entry_widget in entries.items():
        if isinstance(entry_widget, ttk.Combobox):
            ...
            if key == "responsible_person":
                if not entries["responsible_check"].var.get():
                    entry_widget.delete(0, END)
                    entry_widget.config(bg='white')
            elif key == "card_number":
                if not entries["card_check"].var.get():
                    entry_widget.delete(0, END)
                    entry_widget.config(bg='white')
            elif key == "department":
                if not entries["department_check"].var.get():
                    entry_widget.delete(0, END)
                    entry_widget.config(bg='white')
            else:
                entry_widget.delete(0, END)
